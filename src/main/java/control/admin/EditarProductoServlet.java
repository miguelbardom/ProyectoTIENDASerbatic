...
//		response.getWriter().append("Served at: ").append(request.getContextPath());
		...
		if (usuario.getRol_id() != 2 && usuario.getRol_id() != 3) {
			...
			request.getRequestDispatcher("views/admin/editarProducto.jsp").forward(request, response);
		}
...
//		doGet(request, response);
		...
		if (ServletFileUpload.isMultipartContent(request)) {
		    try {
		        ...
		        LibroVO libro = new LibroVO(id, titulo, autor, editorial, isbn, categoriaId, precio, impuesto, stock,
